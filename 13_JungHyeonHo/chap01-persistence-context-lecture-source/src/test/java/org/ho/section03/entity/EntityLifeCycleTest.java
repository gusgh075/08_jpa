...
class EntityLifeCycleTest {
  ...
  void testManagedOtherEntityManager(int menuCode) {
    ...
    // 그러니까 서로 다른 Persistence Context의 1차 캐시 영역에 저장되니까
    // menu1과 menu2는 다른 Persistence Context에서 영속되어있다.
  ...
  void testManagedSameEntityManager(int menuCode) {
    ...
    assertSame(menu1,menu2);  // 둘은 같은 객체이다. 왜냐면 1차캐시되어있는 entity를 참조하기 때문이다.
  }
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    */
    ...
    // then
    // menuPrice는 1000원인데 실제 DB에서는 10000원
    assertNotEquals(
  ...
  @ParameterizedTest
  @CsvSource({"11, 성훈이형의 쿨쿨 죽", "12, 깐딸기 창우형 멬잇"})
